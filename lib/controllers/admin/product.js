...
module.exports = class ProductController {
	...
	static index(req, res) {
		// Retrieve the attribute sets to display at the grid headers
		AttributeSet.where({}).then(cursor => {
			var sets = {};
			cursor.project({'name': 1}).forEach(doc => {
				sets[doc._id] = doc.name;
			});
			page(Product, req).then(data => {
				data.attributeSets = sets;
			...
			Attribute.matchUnique(set, data).then(attrs => {
				Product.with(attrs).then(products => {
					if (products.length) {
						...
						return errCb(`These attributes must be unique: ${msg}`, data);
					}
			...
			AttributeSet.find(product.get('attributeSetId')).then(set => {
				...
				Attribute.matchUnique(set, req.body, product).then(attrs => {
					Product.with(attrs).then(products => {
						if (products.length) {
							var msg = _.map(attrs, a => _.keys(a)).join(', ');
							return errCb(`These attributes must be unique: ${msg}`, req.body);
						}
