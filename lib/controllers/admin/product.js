		...
		AttributeSet.find(attributeSetId).then(set => {
			...
			Attribute.validateUnique(set, data).then(() => {
				...
				_.forEach(req.files, file => {
					// Check it is a valid attribute
					if (file.fieldname === 'gallery') {
						data.gallery.push(`/img/catalog/${file.filename}`);
					}
				});
		...
		Product.find(id).then(product => {
			...
			AttributeSet.find(product.get('attributeSetId')).then(set => {
				...
				Attribute.validateUnique(set, req.body, product).then(() => {
					...
					// Manage images
					var gallery = product.get('gallery');
					console.log('Initial gallery', gallery);
					console.log('Removing', req.body.gallery);
					_.forEach(req.files, file => {
						// Check it is a valid attribute
						if (file.fieldname === 'gallery') {
					...
					if (req.body.gallery) {
						...
						req.body.gallery.forEach(filePath => fs.unlinkSync(`${basePath}${filePath}`));
						_.pullAll(gallery, req.body.gallery); // Remove files
					}
