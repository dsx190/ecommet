...
module.exports = class Model {
	...
	static delete(filter, many) {
		...
		return new Promise((resolve, reject) => {
			Db[func](this.collectionName, filter).then(res => {
				if (!res.deletedCount) return reject();
	...
	/**
	 * Override this function to retrieve the keys of the model
	 * Used for paging (building the data grids) & forms.
	 *
	 * @returns {Array}
	 */
	static get keys() { 
	...
	/**
	 * Shortcut for creating or saving a model.
	 *
	 * @param {Object} data
	 * @param {string} id
	 * @returns {Promise}
	...
	static save(data, id) {
		let classPath = Config.get(`models.${this.collectionName}`);
		if (!classPath) {
			throw new Error('Unable to save: no classPath available');
		}
		var Class = require(classPath);
		return new Promise((resolve, reject) => {
			if (id) {
				Class.update(id, data).then(resolve, reject);
			} else {
				Class.create(data).then(resolve, reject);
			}
		});
	}
